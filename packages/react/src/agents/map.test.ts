...
describe("mapChatStatus", () => {
  it.each<[string, boolean, AgentChatDisplayStatus]>([
    ...
    "maps status %s (recovering=%s) to %s",
    (status, isRecovering, expected) => {
      expect(
        mapChatStatus({ status: status as "submitted", isRecovering }),
      ).toBe(expected);
    },
  );

  it("prefers recovering over thinking and ready", () => {
    expect(mapChatStatus({ status: "submitted", isRecovering: true })).toBe(
      "recovering",
    );
    expect(mapChatStatus({ status: "ready", isRecovering: true })).toBe(
      "recovering",
    );
  });

  it("lets error win over recovering", () => {
    expect(mapChatStatus({ status: "error", isRecovering: true })).toBe(
      "error",
    );
  });
});
...
describe("toAgentMessageParts", () => {
  it("maps text, reasoning, tool, and file parts in order", () => {
    const parts = toAgentMessageParts(
      message([
        ...
        {
          type: "reasoning",
          text: "I should read the documentation.",
          state: "done",
        },
  ...
  it("maps a failed tool part with its error text", () => {
    ...
    expect(tool).toMatchObject({
      type: "tool",
      toolCall: { status: "failed", error: "Timed out" },
    });
  });
  ...
  it("attaches a tool description when provided", () => {
    const [tool] = toAgentMessageParts(
      ...
      {
        toolDescriptions: {
          checkCloudflareDocs: "Read the Workers AI documentation.",
        },
      },
    );
    expect(tool).toMatchObject({
      type: "tool",
      toolCall: { description: "Read the Workers AI documentation." },
    });
  });

  it("defaults tool expansion to open for failed tools, closed otherwise", () => {
    const [failed] = toAgentMessageParts(
      message([toolPart("output-error", "a")]),
    );
    expect(failed).toMatchObject({ toolCall: { expanded: true } });

    const [running] = toAgentMessageParts(
      message([toolPart("input-streaming", "b")]),
    );
  ...
  it("uses expansion overrides and reports toggles by part id", () => {
    ...
    if (reasoning?.type === "reasoning")
      reasoning.reasoning.onExpandedChange?.(false);
    const tool = parts[1];
  ...
  it("derives a filename from a file url when none is provided", () => {
    const [file] = toAgentMessageParts(
      message([
        { type: "file", mediaType: "text/plain", url: "https://x/data.txt" },
      ]),
    );
  ...
  it("wires approval handlers only for approval-requested parts with a callback", () => {
    ...
    const addToolApprovalResponse = (input: {
      id: string;
      approved: boolean;
    }) => {
